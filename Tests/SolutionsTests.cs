using ProjectEulerSolutions.Solutions;
using Xunit;

namespace ProjectEulerSolutions.Tests
{
    public class SolutionsTests
    {
        [Fact]
        public void Solution1()
        {
            Assert.Equal("233168", new Solution1().GetSolution());
        }

        [Fact]
        public void Solution2()
        {
            Assert.Equal("4613732", new Solution2().GetSolution());
        }

        [Fact]
        public void Solution3()
        {
            Assert.Equal("6857", new Solution3().GetSolution());
        }

        [Fact]
        public void Solution4()
        {
            Assert.Equal("906609", new Solution4().GetSolution());
        }

        [Fact]
        public void Solution5()
        {
            Assert.Equal("232792560", new Solution5().GetSolution());
        }

        [Fact]
        public void Solution6()
        {
            Assert.Equal("25164150", new Solution6().GetSolution());
        }

        [Fact]
        public void Solution7()
        {
            Assert.Equal("104743", new Solution7().GetSolution());
        }

        [Fact]
        public void Solution8()
        {
            Assert.Equal("23514624000", new Solution8().GetSolution());
        }

        [Fact]
        public void Solution9()
        {
            Assert.Equal("31875000", new Solution9().GetSolution());
        }

        [Fact]
        public void Solution10()
        {
            Assert.Equal("142913828922", new Solution10().GetSolution());
        }

        [Fact]
        public void Solution11()
        {
            Assert.Equal("70600674", new Solution11().GetSolution());
        }

        [Fact]
        public void Solution12()
        {
            Assert.Equal("76576500", new Solution12().GetSolution());
        }

        [Fact]
        public void Solution13()
        {
            Assert.Equal("5537376230", new Solution13().GetSolution());
        }

        [Fact]
        public void Solution14()
        {
            Assert.Equal("837799", new Solution14().GetSolution());
        }

        [Fact]
        public void Solution15()
        {
            Assert.Equal("137846528820", new Solution15().GetSolution());
        }

        [Fact]
        public void Solution16()
        {
            Assert.Equal("1366", new Solution16().GetSolution());
        }

        [Fact]
        public void Solution17()
        {
            Assert.Equal("21124", new Solution17().GetSolution());
        }

        [Fact]
        public void Solution18()
        {
            Assert.Equal("1074", new Solution18().GetSolution());
        }

        [Fact]
        public void Solution19()
        {
            Assert.Equal("171", new Solution19().GetSolution());
        }

        [Fact]
        public void Solution20()
        {
            Assert.Equal("648", new Solution20().GetSolution());
        }

        [Fact]
        public void Solution21()
        {
            Assert.Equal("31626", new Solution21().GetSolution());
        }

        [Fact]
        public void Solution22()
        {
            Assert.Equal("871198282", new Solution22().GetSolution());
        }

        [Fact]
        public void Solution23()
        {
            Assert.Equal("4179871", new Solution23().GetSolution());
        }

        [Fact]
        public void Solution24()
        {
            Assert.Equal("2783915460", new Solution24().GetSolution());
        }

        [Fact]
        public void Solution25()
        {
            Assert.Equal("4782", new Solution25().GetSolution());
        }

        [Fact]
        public void Solution26()
        {
            Assert.Equal("983", new Solution26().GetSolution());
        }

        [Fact]
        public void Solution27()
        {
            Assert.Equal("-59231", new Solution27().GetSolution());
        }

        [Fact]
        public void Solution28()
        {
            Assert.Equal("669171001", new Solution28().GetSolution());
        }

        [Fact]
        public void Solution29()
        {
            Assert.Equal("9183", new Solution29().GetSolution());
        }

        [Fact]
        public void Solution30()
        {
            Assert.Equal("443839", new Solution30().GetSolution());
        }

        [Fact]
        public void Solution31()
        {
            Assert.Equal("73682", new Solution31().GetSolution());
        }

        [Fact]
        public void Solution32()
        {
            Assert.Equal("45228", new Solution32().GetSolution());
        }

        [Fact]
        public void Solution33()
        {
            Assert.Equal("100", new Solution33().GetSolution());
        }

        [Fact]
        public void Solution34()
        {
            Assert.Equal("40730", new Solution34().GetSolution());
        }

        [Fact]
        public void Solution35()
        {
            Assert.Equal("55", new Solution35().GetSolution());
        }

        [Fact]
        public void Solution36()
        {
            Assert.Equal("7273", new Solution36().GetSolution());
        }

        [Fact]
        public void Solution37()
        {
            Assert.Equal("748317", new Solution37().GetSolution());
        }

        [Fact]
        public void Solution40()
        {
            Assert.Equal("210", new Solution40().GetSolution());
        }

        [Fact]
        public void Solution48()
        {
            Assert.Equal("9110846700", new Solution48().GetSolution());
        }

        [Fact]
        public void Solution67()
        {
            Assert.Equal("7273", new Solution67().GetSolution());
        }
    }
}
